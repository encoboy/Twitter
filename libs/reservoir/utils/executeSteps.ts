  ...
  try {
    ...
    if (json.path && expectedPrice) {
      ...
      if (isSell && quote - expectedPrice < -0.00001) {
        error = {
          type: 'price mismatch',
          message: `The quoted price of ${quote} ETH is less than the expected price of ${expectedPrice} ETH`,
        };
      }
      ...
      if (isBuy && quote - expectedPrice > 0.00001) {
        error = {
          type: 'price mismatch',
          message: `The quoted price of ${quote} ETH is greater than the expected price of ${expectedPrice} ETH`,
        };
      }
    ...
    if (!stepItem.data) {
      ...
      if (!items || !items[incompleteStepItemIndex] || !items[incompleteStepItemIndex].data)
        throw json;
        ...
        if (
          ...
          if (stepItem.txHash && (isSell || isBuy)) {
            ...
            await pollUntilOk(
              ...
              (res) => {
                if (res.status === 200) {
                  const data = res.data as paths['/sales/v3']['get']['responses']['200']['schema'];
        ...
        if (postData) {
          ...
          try {
            const getData = async function () {
              ...
              let response = await axios.post(postOrderUrl.href, JSON.stringify(postData.body), {
